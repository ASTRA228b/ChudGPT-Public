...
Use `!chud web <topic>`, `!chud wiki <topic>`, or `!chud search for <topic>` for a live multi-source lookup. Results can combine DuckDuckGo Instant Answers, English Wikipedia, and—when the query is clearly about software—Stack Overflow. The bot also reads and summarizes text from a public HTTP/HTTPS link pasted into Discord. Its extractive summarizer prioritizes page descriptions and article content while removing navigation, privacy banners, sharing controls, forms, scripts, and repeated boilerplate. Link reading blocks private/local/reserved addresses, credentials, unusual ports, excessive redirects, binary content, and pages over 512 KB. Browsing remains explicit and never runs for ordinary chat messages.
